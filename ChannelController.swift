...
class ChannelController: JSQMessagesViewController {
    ...
    func setupFirebase() {
        ...
        messagesRef = Firebase(url: "https://brilliant-heat-3238.firebaseio.com/room-metadata/")
        
        // *** STEP 4: RECEIVE MESSAGES FROM FIREBASE (limited to latest 25 messages)
        messagesRef.queryLimitedToNumberOfChildren(4).observeEventType(FEventType.ChildAdded, withBlock: { (snapshot) in
            if(!(snapshot.value["id"] == nil)){
                let id = snapshot.value["id"] as? String
                let name = snapshot.value["name"] as? String
                
                let message = Message(channel: id, sender: name)
